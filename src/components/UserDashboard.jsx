...
const UserDashboard = ({ user }) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        // Add a small delay to ensure authentication propagates
        ...
        if (userDoc.exists()) {
          ...
          // Store weighted average score in userData for easy access
          const weightedAverage = calculateWeightedAverageScore(sortedResults);
          setUserData(prev => ({ ...prev, weightedAverageScore: weightedAverage, examResults: sortedResults }));
          ...
          console.log('User document does not exist for user:', user.uid);
        ...
        // Handle permission denied without signing out
    ...
    // Add delay before calling fetchUserData
  ...
  useEffect(() => {
    ...
    // Add delay and ensure userData is available
    if (userData && !loading) {
      const timer = setTimeout(() => {
        fetchNotifications();
      }, 300);
  ...
  const addToRecentActivity = useCallback(async (action, description) => {
    if (!user) {
      console.log('No user available for adding recent activity');
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
          ...
          <p className="text-lg font-semibold text-gray-800 mt-1">
            {examResults.length > 0 ? calculateWeightedAverageScore(examResults) : 0}%
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
            ...
            <div className="text-2xl font-bold text-green-600">
              {examResults.length > 0
                ? calculateWeightedAverageScore(examResults)
                : 0}%
            </div>
          </div>
        ...
        {examResults.length > 0 && (
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="flex items-center space-x-2 mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-900">Average Score Calculation</h3>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-3">Score Breakdown by Category</h4>
                  <div className="space-y-2">
                    {(() => {
                      ...
                      examResults.forEach(result => {
                        ...
                        categoryStats[result.category].totalWeight += result.totalQuestions || 1;
                      });

                      return Object.entries(categoryStats).map(([category, stats]) => {
                        const categoryAverage = stats.scores.reduce((sum, score) => sum + score, 0) / stats.scores.length;
                        return (
                          <div key={category} className="flex justify-between items-center py-1">
                            ...
                            <div className="text-right">
                              ...
                              <span className="text-xs text-gray-500 ml-1">
                                ({stats.scores.length} exam{stats.scores.length !== 1 ? 's' : ''})
                              </span>
                            </div>
                          </div>
                        );
                ...
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-3">How It's Calculated</h4>
                  <div className="text-sm text-gray-600 space-y-2">
                    <div className="bg-white p-3 rounded border">
                      <p className="font-mono text-xs mb-2">
                        Weighted Average = (Σ(Score × Questions)) / Σ(Questions)
                      </p>
                      <div className="space-y-1 text-xs">
                        <p><strong>Total Weighted Points:</strong> {examResults.reduce((sum, r) => sum + (r.score * (r.totalQuestions || 1)), 0)}</p>
                        <p><strong>Total Questions:</strong> {examResults.reduce((sum, r) => sum + (r.totalQuestions || 1), 0)}</p>
                        <p><strong>Final Average:</strong> {calculateWeightedAverageScore(examResults)}%</p>
                      </div>
                    </div>
                    <p className="text-xs text-gray-500">
                      This method gives more weight to exams with more questions, providing a fairer representation of your overall performance.
                    </p>
                  </div>
                </div>
              </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3">
                        ...
                        <div className="flex items-center space-x-2 mt-2 sm:mt-0">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${result.score >= 80 ? 'bg-green-100 text-green-800' :
                            ...
                            {result.score >= 80 ? 'Excellent' : result.score >= 60 ? 'Passed' : 'Needs Improvement'}
                          </span>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-bold text-gray-800 capitalize">
                            {result.difficulty || 'Professional Level'}
                          </p>
                        </div>
                      </div>
          ...
          {sortedAndFilteredResults.length > 0 && (
            <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
              <div className="flex items-center justify-between text-sm text-gray-600">
                ...
                <span>
                  Overall Average: {calculateWeightedAverageScore(examResults)}%
                </span>
              </div>
            </div>
          )}
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <h1 className="text-xl font-bold text-gray-800">Reviewer WebApp</h1>
              </div>
            </div>
        ...
        <nav className={`fixed inset-y-0 left-0 z-50 lg:z-30 w-64 bg-white border-r border-gray-200 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
          ...
          <div className="p-4 space-y-2 pt-20"> {/* Changed: Always use pt-20 for both mobile and desktop */}
            ...
            <button
              ...
              className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-left transition-colors ${activeTab === 'downloadables'
                ? 'bg-indigo-100 text-indigo-700'
                : 'text-gray-600 hover:bg-gray-100'
                }`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-left transition-colors ${activeTab === 'about'
                ? 'bg-indigo-100 text-indigo-700'
                : 'text-gray-600 hover:bg-gray-100'
                }`}
            >
